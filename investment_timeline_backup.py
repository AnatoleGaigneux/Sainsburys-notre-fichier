...
X=time
Y=BAU_operating_cost-Cum_financial_savings
plt.plot(X, Y)
plt.ylabel('Stores operating cost timeline £')
plt.xlabel('time (years)')

Op_cost_reduction = round(Cum_financial_savings[-1]/BAU_operating_cost*100,1)
plt.annotate('%s %%' %(100-Op_cost_reduction), (X[-2],Y[-5]))
...
plt.figure(2)
x=time
y=init_emissions-Cum_carbon_savings
plt.plot(x,y)
plt.xlabel('time (years)')
plt.ylabel('Emissions (tCO2)')
plt.ylim(0,1200000)

Emission_reduction_tot_footprint = round(Cum_carbon_savings[-1]/init_emissions*100,1)
Emission_reduction_BAU = (Cum_carbon_savings[-1]/BAU_emissions*100)

plt.annotate('%s %%' %(100-Emission_reduction_tot_footprint), (x[-2],y[-10]))
