...
Y1_Nicco = np.array(target_train )   
   
regr = linear_model.LinearRegression()



slope, intercept, r_value, p_value, std_err = stats.linregress(X1_Nicco,Y1_Nicco)

Y1_pred = X1_Nicco*slope + intercept


plt.figure()
plt.plot(X1_Nicco,Y1_Nicco, "ro")
plt.plot(X1_Nicco,Y1_pred)
#plt.plot(X1_Nicco,X2_Nicco, "bo")
plt.show



#regr.fit(X1_Nicco, Y1_Nicco)
#y_predict = regr.predict(X1_Nicco)
...
##Make predictions using the testing sets
#            
#target_pred = lm.predict(X1_test)
#
##get coefficents
#print('Coefficients: \n', lm.coef_)
##print mean squared error
...
##Plot outputs
#
#plt.scatter(X1_test, target_test, 'ro')
#plt.plot(X1_train, target_pred)
#'''
#fig = plt.figure(1)
#ax = Axes3D(fig)
#ax.scatter(X1_test, X2_test, target_test, zdir='z', s=20, c='r', depthshade=True)
#ax.scatter(X1_train, X2_train, target_train, zdir='z', s=20, c='r', depthshade=True)
#X1_range = np.arange(min(Area),max(Area))
#X2_range =  np.arange(min(h2p),max(h2p))
#params = lm.coef_
#Y = X1_range*params[0] + X2_range*params[1]+lm.intercept_
